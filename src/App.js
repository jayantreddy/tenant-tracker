...
const genTenants = () => Array.from({length:40},(_,i)=>{
  ...
  const rents = india ? [8000,9500,10000,11000,12000,12500,13000,14000,15000] : [800,950,1000,1100,1200,1250,1300,1400,1500];
  const rent = rents[i%9];
  ...
  return {id:i+1,unit:String(bi+1)+"0"+String((i%10)+1),name:"Tenant "+(i+1),phone,email:"tenant"+(i+1)+"@email.com",rent,leaseEnd:leaseEnd.toISOString().slice(0,10),buildingId:bld.id};
});

const sharedInp = {border:"1px solid #E8EAED",borderRadius:8,padding:"9px 12px",fontSize:14,background:"#fff",color:"#1A1D23",width:"100%",boxSizing:"border-box",outline:"none"};
  ...
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#1E2330,#2D3550)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"-apple-system,sans-serif"}}>
      <div style={{background:"#fff",borderRadius:16,padding:"40px 36px",width:360,boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}}>
        ...
        {["Username","Password"].map((lbl,idx)=>(
          <div key={lbl} style={{marginBottom:14}}>
            <label style={{fontSize:12,fontWeight:600,color:"#6B7280",display:"block",marginBottom:6}}>{lbl}</label>
            <input type={idx===1?"password":"text"} value={idx===0?user:pwd}
              onChange={e=>idx===0?setUser(e.target.value):setPwd(e.target.value)}
              onKeyDown={e=>{setErr("");if(e.key==="Enter")attempt();}}
              placeholder={"Enter your "+lbl.toLowerCase()} style={sharedInp}/>
          </div>
        ))}
...
function ChangePwdModal({user,onClose}) {
  const [fields,setFields] = useState({cur:"",n1:"",n2:""});
  const [err,setErr] = useState("");
  const [ok,setOk] = useState(false);
  const save = () => {
    const pwds = JSON.parse(localStorage.getItem("manne_pwds")||"{}");
    if (fields.cur !== (pwds[user]||DEFAULT_PWD)) { setErr("Current password incorrect."); return; }
    if (fields.n1.length < 6) { setErr("New password must be 6+ characters."); return; }
    if (fields.n1 !== fields.n2) { setErr("Passwords do not match."); return; }
    pwds[user] = fields.n1;
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{background:"#fff",borderRadius:14,padding:28,width:340,boxShadow:"0 10px 40px rgba(0,0,0,0.2)"}}>
        ...
        {ok
          ...
          : [["cur","Current password"],["n1","New password"],["n2","Confirm new password"]].map(([k,lbl])=>(
              <div key={k} style={{marginBottom:12}}>
                <label style={{fontSize:12,fontWeight:600,color:"#6B7280",display:"block",marginBottom:5}}>{lbl}</label>
                <input type="password" value={fields[k]} onChange={e=>{setFields(f=>({...f,[k]:e.target.value}));setErr("");}} style={sharedInp}/>
              </div>
            ))
        }
...
function Donut({slices,centerTop,centerBot,size=260,outerR,innerR,labels=true}) {
  ...
  const paths = slices.map(s=>{
    ...
    if (single && s.value===tot) return {full:true,...s};
    const st=(cum/tot)*Math.PI*2-Math.PI/2; cum+=s.value;
    const en=(cum/tot)*Math.PI*2-Math.PI/2;
    const mid=(st+en)/2;
    const cos1=Math.cos(st),sin1=Math.sin(st),cos2=Math.cos(en),sin2=Math.sin(en);
    const lg=s.value/tot>0.5?1:0;
    const d="M"+(cx+ri*cos1)+","+(cy+ri*sin1)+" L"+(cx+r*cos1)+","+(cy+r*sin1)+" A"+r+","+r+" 0 "+lg+",1 "+(cx+r*cos2)+","+(cy+r*sin2)+" L"+(cx+ri*cos2)+","+(cy+ri*sin2)+" A"+ri+","+ri+" 0 "+lg+",0 "+(cx+ri*cos1)+","+(cy+ri*sin1)+"Z";
    const lx=cx+(r+24)*Math.cos(mid), ly=cy+(r+24)*Math.sin(mid);
    const pct=Math.round(s.value/tot*100);
    return {d,color:s.color,value:s.value,lx,ly,mid,pct};
  }).filter(Boolean);
  const fmtV=v=>v>=1000000?"$"+(v/1000000).toFixed(1)+"M":v>=1000?"$"+(v/1000).toFixed(0)+"K":"$"+v;
  return (
    <svg width={size} height={size} viewBox={"0 0 "+size+" "+size} style={{overflow:"visible"}}>
      {paths.map((p,i)=>p.full
        ? <g key={i}><circle cx={cx} cy={cy} r={r} fill={p.color}/><circle cx={cx} cy={cy} r={ri} fill="#fff"/></g>
        : <g key={i}>
            <path d={p.d} fill={p.color} stroke="#fff" strokeWidth={3}/>
            {labels && p.pct>=6 && <>
              ...
              <text x={p.lx} y={p.ly-3} textAnchor="middle" style={{fontSize:10,fontWeight:700,fill:p.color}}>{fmtV(p.value)}</text>
              <text x={p.lx} y={p.ly+10} textAnchor="middle" style={{fontSize:9,fill:"#9CA3AF"}}>{p.pct}%</text>
            </>}
          </g>
      )}
      {centerTop && <text x={cx} y={cy-6} textAnchor="middle" style={{fontSize:size*0.1,fontWeight:800,fill:"#1A1D23"}}>{centerTop}</text>}
      {centerBot && <text x={cx} y={cy+size*0.07} textAnchor="middle" style={{fontSize:size*0.045,fill:"#6B7280"}}>{centerBot}</text>}
    </svg>
  );
}
...
function Dashboard({currentUser,onLogout}) {
  ...
  const addRepair = (tid,text,cat) => { if(!text.trim())return; setRepairs(p=>({...p,[tid]:[...(p[tid]||[]),{id:Date.now(),text:text.trim(),cat:cat||"Other",date:now.toISOString().slice(0,10),status:"open"}]})); };
  const toggleRep = (tid,iid) => setRepairs(p=>({...p,[tid]:p[tid].map(i=>i.id===iid?{...i,status:i.status==="open"?"resolved":"open"}:i)}));
  const upDoc = (setter,id,files) => { const docs=Array.from(files).map(f=>({name:f.name,url:URL.createObjectURL(f),date:now.toISOString().slice(0,10)})); setter(p=>({...p,[id]:[...(p[id]||[]),...docs]})); };
  ...
  const exportCSV = () => {
    ...
    tenants.forEach(t=>{const p=getPay(t.id);const b=buildings.find(x=>x.id===t.buildingId);rows.push([b?.name,t.unit,t.name,t.rent,b?.currency||"INR",p.status,p.amount||"",p.date,t.leaseEnd]);});
  ...
  const btn = v=>{const base={borderRadius:8,padding:"7px 14px",cursor:"pointer",fontSize:13,fontWeight:500,border:"1px solid transparent",whiteSpace:"nowrap"};return v==="primary"?{...base,background:C.primary,color:"#fff"}:v==="green"?{...base,background:C.green,color:"#fff"}:v==="red"?{...base,background:"#fff",color:C.red,border:"1px solid "+C.border}:{...base,background:"#fff",color:C.text,border:"1px solid "+C.border};};
    ...
    return (
      <div style={{...card,marginBottom:20}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          {[["name","Name","text"],["phone","Phone","text"],["email","Email","email"],["rent","Monthly rent ("+cur.symbol+")","number"],["leaseEnd","Lease end date","date"]].map(([k,lbl,type])=>(
            <div key={k}>
        ...
        <div style={{marginTop:18,paddingTop:16,borderTop:"1px solid "+C.borderLt}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <label style={{fontSize:13,fontWeight:600}}>Documents</label>
  ...
  function PanelEditBuilding({data}) {
    ...
    const save = () => {
      if(!f.name?.trim()||!f.address?.trim()){alert("Fill in name and address.");return;}
        ...
        <div style={{padding:"28px 32px",maxWidth:1100}}>
          ...
          {view==="dashboard" && <>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:16,marginBottom:24}}>
              {[["Expected",totalExp,C.text],["Collected",totalColl,C.green],["Pending",totalExp-totalColl,C.red]].map(([l,v,c])=>(
                <div key={l} style={{...card,padding:"18px 20px"}}>
                  <div style={{fontSize:13,color:C.text2,marginBottom:8,fontWeight:500}}>{l} rent</div>
                  <div style={{fontSize:22,fontWeight:700,color:c,fontSize:18}}>Mixed currencies</div>
                  <div style={{fontSize:13,color:C.text2,marginTop:4}}>{buildings.map((b,bi)=>{
                    const bT=tenants.filter(t=>t.buildingId===b.id);
                    const bV=bT.reduce((s,t)=>{const p=getPay(t.id);const paid=p.status==="paid"?t.rent:p.status==="partial"?Number(p.amount)||0:0;const exp=t.rent;return s+(l==="Expected"?exp:l==="Collected"?paid:exp-paid);},0);
                    return <div key={b.id}>{b.name.split(" ")[0]}: {fmtAmt(bV,b.id)}</div>;
                  })}</div>
                </div>
              ))}
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
                    <div style={{display:"flex",gap:12,alignItems:"center"}}>
                      ...
                      <div>
                        ...
                        <span style={{background:C.primaryLt,color:C.primary,borderRadius:5,padding:"1px 8px",fontSize:11,fontWeight:600}}>{cur.code} {cur.symbol}</span>
                      </div>
                    </div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:24}}>
                <div style={card}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
                    <Donut size={160} slices={[{value:openN,color:C.red},{value:resN,color:C.green}]} centerTop={String(allRepairs.length)} centerBot="total" labels={false}/>
                ...
                <div style={card}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
                    <Donut size={160} slices={byCat.length?byCat:[{value:1,color:"#E5E7EB"}]} centerTop={String(allRepairs.length)} centerBot="repairs" labels={false}/>
                    <div>
